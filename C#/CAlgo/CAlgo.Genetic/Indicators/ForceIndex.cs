...
{
    [Indicator(ScalePrecision = 5, AutoRescale = false, IsOverlay = false, AccessRights = AccessRights.None)]
    [Levels()]
    public class ForceIndex : Indicator
    {
        Mq4Double Mq4Init()
        {
            Mq4String sShortName = "";
        ...
        {
            Mq4Double i = 0;
            Mq4Double nCountedBars = 0;
            Mq4Double nLimit = 0;

            nCountedBars = IndicatorCounted();
            if (Bars <= ExtForcePeriod)
                return 0;
            if (nCountedBars > ExtForcePeriod)
                nCountedBars--;
        ...
        [Parameter("ExtForcePeriod", DefaultValue = 13)]
        public int ExtForcePeriod_parameter { get; set; }
        bool _ExtForcePeriodGot;
        Mq4Double ExtForcePeriod_backfield;
        Mq4Double ExtForcePeriod
        {
            get
            {
                if (!_ExtForcePeriodGot)
                    ExtForcePeriod_backfield = ExtForcePeriod_parameter;
                return ExtForcePeriod_backfield;
            }
            set { ExtForcePeriod_backfield = value; }
        }

        [Parameter("ExtForceMAMethod", DefaultValue = 0)]
        public int ExtForceMAMethod_parameter { get; set; }
        bool _ExtForceMAMethodGot;
        Mq4Double ExtForceMAMethod_backfield;
        Mq4Double ExtForceMAMethod
        {
            get
            {
                if (!_ExtForceMAMethodGot)
                    ExtForceMAMethod_backfield = ExtForceMAMethod_parameter;
                return ExtForceMAMethod_backfield;
            }
            set { ExtForceMAMethod_backfield = value; }
        }

        [Parameter("ExtForceAppliedPrice", DefaultValue = 0)]
        public int ExtForceAppliedPrice_parameter { get; set; }
        bool _ExtForceAppliedPriceGot;
        Mq4Double ExtForceAppliedPrice_backfield;
        Mq4Double ExtForceAppliedPrice
        {
            get
            {
                if (!_ExtForceAppliedPriceGot)
                    ExtForceAppliedPrice_backfield = ExtForceAppliedPrice_parameter;
                return ExtForceAppliedPrice_backfield;
            }
            set { ExtForceAppliedPrice_backfield = value; }
        }

        [Output("ExtForceBuffer", Color = Colors.DodgerBlue, PlotType = PlotType.DiscontinuousLine)]
        public IndicatorDataSeries ExtForceBuffer_AlgoOutputDataSeries { get; set; }

        int indicator_buffers = 1;
        int indicator_color1 = DodgerBlue;
        ...
        private Mq4OutputDataSeries ExtForceBuffer;
        ...
        protected override void Initialize()
        {
            ...
            if (ExtForceBuffer_AlgoOutputDataSeries == null)
                ExtForceBuffer_AlgoOutputDataSeries = CreateDataSeries();
            ExtForceBuffer = new Mq4OutputDataSeries(this, ExtForceBuffer_AlgoOutputDataSeries, ChartObjects, 0, 0, () => CreateDataSeries(), 1, Colors.DodgerBlue);
            AllBuffers.Add(ExtForceBuffer);
            AllOutputDataSeries.Add(ExtForceBuffer_AlgoOutputDataSeries);
            CommonInitialize();
        }
        ...
        public override void Calculate(int index)
        {
            ...
            {
                _currentIndex = index;
                ExtForceBuffer.SetCurrentIndex(index);
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly ForceIndex _indicator;

            public Mq4OutputDataSeries(ForceIndex indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = ForceIndex.GetArrowByCode(ArrowCode);
