...
extern double stoploss = 6;                // stop loss
...
double tp2percentage = 0.25;
double takeprofit3 = 20;
double tp3percentage = 0.5;
extern double trailingstart = 6;           // profit in pips required to enable the trailing stop
extern double trailingstop = 6;            // trailing stop
...
bool changedirection = false;              // only buy after a sell order, sell after a buy order
bool onesideatatime = false;               // enter only long or short when a long or short is already opened
...
extern double brokermode = 0;              // 0=we consider only 3/4 digits of price, 1=we consider all digits
extern double distance = 1;                // minimum distance in pips between price and previous price to consider a movement
extern bool long=true;                   // trade long orders
extern int movements1 = 4;                 // movements to consider for long orders
extern double seconds1 = 3;                // seconds to consider for long orders
extern bool short=true;                  // trade short orders
extern int movements2 = 4;                 // movements to consider for short orders
extern double seconds2 = 3;                // seconds to consider for short orders
int shift = 1;                      // bar in the past to take in consideration for the signal
...
int i, bc = -1, cnt, tpb, tps, tries = 100, lastorder, buyorderprofit, sellorderprofit, lotsize, losses, sleep, xmartingalemode = 0, mstop;
...
double lastbuyopenprice, lastsellopenprice, lastbuyprofit, lastsellprofit, tradeprofit, buyorderpips, sellorderpips, lossestorecover = 0, globallosses;
...
double pt, pt2, mt, pricedigits;

int longmovements, shortmovements, timelong, timeshort;
bool settimelong, settimeshort, addstoploss;
double previousupprice, previousdownprice;
...
int init()
{
    ...
    if (Digits == 3 || Digits == 5)
    {
        pt = Point * 10;
        mt = 10;
        if (brokermode == 0) { pricedigits = Digits - 1; pt2 = pt; }
        if (brokermode == 1) { pricedigits = Digits; pt2 = Point; }
    }
    else
    {
        pt = Point;
        mt = 1;
        if (brokermode == 0 || brokermode == 1) { pricedigits = Digits; pt2 = pt; }
    }
    if (stop > 0)
    {
        stoploss = stop;
...
int start()
{
    ...
    */
    ...
    if (onetime || shift == 0)
    {
        ...
        if (total == 0)
        {
            if (previousupprice <= NormalizeDouble(Ask, pricedigits) - distance * pt2)
            {
                longmovements = longmovements + 1;
                previousupprice = NormalizeDouble(Ask, pricedigits);
                if (settimelong == false)
                {
                    timelong = TimeCurrent();
                    settimelong = true;
                }
            }
            else
            ...
            if (previousdownprice >= NormalizeDouble(Ask, pricedigits) + distance * pt2)
            {
                shortmovements = shortmovements + 1;
                previousdownprice = NormalizeDouble(Ask, pricedigits);
                if (settimeshort == false)
                {
                    timeshort = TimeCurrent();
                    settimeshort = true;
                }
            }
            else
            ...
            if (previousupprice == 0)
            {
                previousupprice = NormalizeDouble(Ask, pricedigits);
            }
            if (previousdownprice == 0)
            {
                previousdownprice = NormalizeDouble(Ask, pricedigits);
            }
        }
        ...
        int signal = 0;
        if (long && longmovements == movements1 || shortmovements == movements2 && longmovements >= shortmovements && TimeCurrent() - timelong <= seconds1) signal = 1;
        if (short && longmovements == movements1 || shortmovements == movements2 && longmovements < shortmovements && TimeCurrent() - timeshort <= seconds2) signal = 2;
    ...
    if (addpositions)
    {
        ...
        if (totalbuy > 0)
        {
            if ((addposmode == 0 && Ask <= lastbuyopenprice - pipstep * pt * MathPow(pipstepfactor, totalbuy))
...
&& (eoobexceptaddpos || (eoobexceptaddpos == false && barstatus))) buy = true;
        ...
        if (totalsell > 0)
        {
            if ((addposmode == 0 && Bid >= lastsellopenprice + pipstep * pt * MathPow(pipstepfactor, totalsell))
...
&& (eoobexceptaddpos || (eoobexceptaddpos == false && barstatus))) sell = true;
    ...
    if (sell) if (oppositeclose) if (partialexit) partialclose(OP_BUY, 2); else close(OP_BUY);
    if (buy) if (oppositeclose) if (partialexit) partialclose(OP_SELL, 2); else close(OP_SELL);
    ...
    if (partialclose) { partialclose(OP_BUY, 1); partialclose(OP_SELL, 1); }
    if (partialclose) { partialclose2(OP_BUY, 1); partialclose2(OP_SELL, 1); }
    if (xmartingalemode == 1 && totalprofit >= -1 * lossestorecover) { close(OP_BUY); close(OP_SELL); }
    ...
    if (sell && tps < tradesperbar && !onlybuy)
    {
        ...
        if (!ecn)
        {
            ...
            if (reverseatstop) if (time7 != Time[0]) { if (rasoppositeclose) delete(OP_SELLSTOP); RefreshRates(); ticket = open(OP_BUYSTOP, blots, Ask + stoploss * pt, rasstoploss, rastakeprofit, expire, Red); time7 = Time[0]; }
        }
        if (ecn)
        {
            if (instantorders)
            ...
            if (reverseatstop) if (time7 != Time[0]) { if (rasoppositeclose) delete(OP_SELLSTOP); RefreshRates(); ticket = open(OP_BUYSTOP, blots, Ask + stoploss * pt, 0, 0, expire, Red); time7 = Time[0]; }
        }
...
int open(int type, double lots, double price, double stoploss, double takeprofit, int expire, color clr)
{
    ...
    comment + " " + DoubleToStr(magic, 0), magic, expire, clr);
    return (ticket);
}
...
void close(int type)
{
    if (OrdersTotal() > 0)
    {
        for (i = OrdersTotal() - 1; i >= 0; i--)
        {
            ...
            {
                if (OrderSymbol() == Symbol() && OrderMagicNumber() == magic
             && (oppositecloseminloss == 0 || (oppositecloseminloss > 0 && (OrderProfit() / (OrderLots() * 10)) < -1 * oppositecloseminloss)))
                {
                    RefreshRates();
...
void partialclose(int type, int mode)
{
    if (OrdersTotal() > 0)
    {
        for (i = OrdersTotal() - 1; i >= 0; i--)
        {
            ...
            if (type == OP_BUY && OrderType() == OP_BUY)
            {
                if (OrderSymbol() == Symbol() && OrderMagicNumber() == magic && OrderLots() >= lots
                && (mode == 1 && OrderProfit() >= takeprofit2 * OrderLots() * 10) || mode == 2)
                {
                    RefreshRates();
                    OrderClose(OrderTicket(), OrderLots() * 0.5, NormalizeDouble(Bid, Digits), slippage * mt);
                }
            }
            if (type == OP_SELL && OrderType() == OP_SELL)
            {
                ...
                {
                    RefreshRates();
                    OrderClose(OrderTicket(), OrderLots() * 0.5, NormalizeDouble(Ask, Digits), slippage * mt);
                }
            }
            if (type == 3)
            {
                if (OrderSymbol() == Symbol() && OrderMagicNumber() == magic && OrderType() == OP_BUY && OrderLots() >= lots
                && (mode == 1 && OrderProfit() >= takeprofit2 * OrderLots() * 10) || mode == 2)
                {
                    RefreshRates();
                    OrderClose(OrderTicket(), OrderLots() * 0.5, NormalizeDouble(Bid, Digits), slippage * mt);
                }
                if (OrderSymbol() == Symbol() && OrderMagicNumber() == magic && OrderType() == OP_SELL && OrderLots() >= lots
                && (mode == 1 && OrderProfit() >= takeprofit2 * OrderLots() * 10) || mode == 2)
                {
                    RefreshRates();
                    OrderClose(OrderTicket(), OrderLots() * 0.5, NormalizeDouble(Ask, Digits), slippage * mt);
...
void partialclose2(int type, int mode)
{
    if (OrdersTotal() > 0)
    {
        for (i = OrdersTotal() - 1; i >= 0; i--)
        {
            ...
            {
                if (OrderSymbol() == Symbol() && OrderMagicNumber() == magic && OrderLots() < lots
                && (mode == 1 && OrderProfit() >= takeprofit3 * OrderLots() * 10) || mode == 2)
                {
                    RefreshRates();
                    OrderClose(OrderTicket(), NormalizeDouble(OrderLots() * tp3percentage, lotdigits), NormalizeDouble(Bid, Digits), slippage * mt);
                }
            }
            if (type == OP_SELL && OrderType() == OP_SELL)
            {
                ...
                {
                    RefreshRates();
                    OrderClose(OrderTicket(), NormalizeDouble(OrderLots() * tp3percentage, lotdigits), NormalizeDouble(Ask, Digits), slippage * mt);
                }
            }
            if (type == 3)
            {
    ...
    for (i = OrdersTotal(); i >= 0; i--)
    {
        ...
        if (OrderSymbol() == Symbol() && OrderMagicNumber() == magic)
        {
            if (OrderType() == OP_BUY)
            {
                ...
                {
                    RefreshRates();
                    OrderModify(OrderTicket(), OrderOpenPrice(), NormalizeDouble(Ask - stoploss * pt, Digits), OrderTakeProfit(), 0, Red);
                }
                if (OrderTakeProfit() == 0 && stoploss == 0 && takeprofit > 0)
                {
                    RefreshRates();
                    OrderModify(OrderTicket(), OrderOpenPrice(), OrderStopLoss(), NormalizeDouble(Ask + takeprofit * pt, Digits), 0, Red);
                }
                if (OrderTakeProfit() == 0 && OrderStopLoss() == 0 && stoploss > 0 && takeprofit > 0)
                {
                    RefreshRates();
                    OrderModify(OrderTicket(), OrderOpenPrice(), NormalizeDouble(Ask - stoploss * pt, Digits), NormalizeDouble(Ask + takeprofit * pt, Digits), 0, Red);
                }
            }
            if (OrderType() == OP_SELL)
            {
                ...
                {
                    RefreshRates();
                    OrderModify(OrderTicket(), OrderOpenPrice(), OrderStopLoss(), NormalizeDouble(Bid - takeprofit * pt, Digits), 0, Red);
                }
                if (OrderTakeProfit() == 0 && OrderStopLoss() == 0 && stoploss > 0 && takeprofit > 0)
                {
                    RefreshRates();
                    OrderModify(OrderTicket(), OrderOpenPrice(), NormalizeDouble(Bid + stoploss * pt, Digits), NormalizeDouble(Bid - takeprofit * pt, Digits), 0, Red);
                }
            }
        }
    }
