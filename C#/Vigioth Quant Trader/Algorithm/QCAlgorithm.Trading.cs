...
namespace VigiothCapital.QuantTrader.Algorithm
{
    public partial class QCAlgorithm
    {
        ...
        private OrderResponse PreOrderChecksImpl(SubmitOrderRequest request)
        {
            ...
            if (request.Quantity == 0 || request.Symbol == null || request.Symbol == VigiothCapital.QuantTrader.Symbol.Empty)
            {
        ...
        public List<int> Liquidate(Symbol symbolToLiquidate = null)
        {
            ...
            symbolToLiquidate = symbolToLiquidate ?? VigiothCapital.QuantTrader.Symbol.Empty;

            foreach (var symbol in Securities.Keys.OrderBy(x => x.Value))
            {
                // symbol not matching, do nothing
                if (symbol != symbolToLiquidate && symbolToLiquidate != VigiothCapital.QuantTrader.Symbol.Empty) 
                    continue;
