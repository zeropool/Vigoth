...
using SecurityTypeMarket = System.Tuple<VigiothCapital.QuantTrader.SecurityType, string>;
...
namespace VigiothCapital.QuantTrader.Algorithm
{
    ...
    public partial class QCAlgorithm : MarshalByRefObject, IAlgorithm
    {
        ...
        private Symbol _benchmarkSymbol = VigiothCapital.QuantTrader.Symbol.Empty;
        ...
        /// </summary>
        /// <remarks>This property is set with SetStartDate() and defaults to the earliest VigiothCapital.QuantTrader data available - Jan 1st 1998. It is ignored during live trading </remarks>
        ...
        public void PostInitialize()
        {
            ...
            if (Benchmark == null)
            {
                ...
                if (_benchmarkSymbol == null || _benchmarkSymbol == VigiothCapital.QuantTrader.Symbol.Empty)
                {
                    _benchmarkSymbol = VigiothCapital.QuantTrader.Symbol.Create("SPY", SecurityType.Equity, Market.USA);
                }
            ...
            foreach (var option in Securities.Select(x => x.Value).OfType<Option>())
            {
                ...
                var underlying = VigiothCapital.QuantTrader.Symbol.Create(sid.Symbol, SecurityType.Equity, sid.Market);
                Security equity;
        ...
        public void SetBenchmark(SecurityType securityType, string symbol)
        {
            ...
            _benchmarkSymbol = VigiothCapital.QuantTrader.Symbol.Create(symbol, securityType, market);
        }
        ...
        public void SetBenchmark(string symbol)
        {
            ...
            _benchmarkSymbol = security == null 
                ? VigiothCapital.QuantTrader.Symbol.Create(symbol, SecurityType.Equity, Market.USA)
                : security.Symbol;
        }
        ...
        public void SetLiveMode(bool live) 
        {
            if (!_locked)
            {
                ...
                if (live)
                {
                    ...
                    _endDate = VigiothCapital.QuantTrader.Time.EndOfTime;
                }
            }
        }

        /// <summary>
        /// Add specified data to our data subscriptions. VigiothCapital.QuantTrader will funnel this data to the handle data routine.
        /// </summary>
        ...
        public Security AddSecurity(SecurityType securityType, string symbol, Resolution resolution, string market, bool fillDataForward, decimal leverage, bool extendedMarketHours)
        {
            try
            {
                ...
                {
                    symbolObject = VigiothCapital.QuantTrader.Symbol.Create(symbol, securityType, market);
                }
        ...
        public Option AddOption(string underlying, Resolution resolution = Resolution.Minute, string market = Market.USA, bool fillDataForward = true, decimal leverage = 0m)
        {
            ...
            {
                canonicalSymbol = VigiothCapital.QuantTrader.Symbol.Create(underlying, SecurityType.Option, market, alias);
            }
        ...
        /// </summary>
        /// <remarks>Intended for internal use by the VigiothCapital.QuantTrader.Engine only.</remarks>
            ...
            where T : Security
        {
            ...
            {
                symbol = VigiothCapital.QuantTrader.Symbol.Create(ticker, securityType, market);
            }
